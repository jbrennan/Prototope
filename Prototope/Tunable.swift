...
public func tunable(_ defaultValue: Double, name: String, min: Double? = nil, max: Double? = nil) -> Double {
	if let specItem = defaultSpec._KFSpecItem(forKey: name) {
		return (specItem.objectValue as! NSNumber).doubleValue
	} else {
        let minValue = min ?? 0
        let maxValue = max ?? 2 * defaultValue
		defaultSpec.addDoubleItem(forKey: name, defaultValue: defaultValue, minValue: minValue, maxValue: maxValue)
		return defaultValue
	}
}
...
public func tunable(_ defaultValue: Double, name: String, min: Double? = nil, max: Double? = nil, changeHandler: @escaping (Double) -> Void) {
    tunable(defaultValue, name: name, min: min, max: max) // Make sure it exists
	defaultSpec.withDoubleForKey(name, owner: UIApplication.shared, maintain: { owner, value in changeHandler(value) })
}

/** Returns the tunable value stored in TunableValues.json under `name`. If none is present, returns `defaultValue`. */
public func tunable(_ defaultValue: Bool, name: String) -> Bool {
	if let specItem = defaultSpec._KFSpecItem(forKey: name) {
		return (specItem.objectValue as! NSNumber).boolValue
	} else {
		defaultSpec.addBoolSpecItem(forKey: name, defaultValue: defaultValue)
		return defaultValue
	}
}

/** Whenever the named tunable value changes, runs `changeHandler` with the new value. Runs `changeHandler` once initially with the existing or default value. */
public func tunable(_ defaultValue: Bool, name: String, changeHandler: @escaping (Bool) -> Void) {
	tunable(defaultValue, name: name) // Make sure it exists
	defaultSpec.withBoolForKey(name, owner: UIApplication.shared, maintain: { owner, value in changeHandler(value) })
}

/** Returns the tunable value stored in TunableValues.json under `name`. If none is present, returns `defaultValue`. */
public func tunable(_ defaultValue: Point, name: String) -> Point {
	return Point(
...
/** Whenever the named tunable value changes, runs `changeHandler` with the new value. Runs `changeHandler` once initially with the existing or default value. */
public func tunable(_ defaultValue: Point, name: String, changeHandler: @escaping (Point) -> Void) {
